...
var DirectoryEntry = function(name, key, type, date, size) {
  ...
  return { 
    ...
    icon: function() {
      if (this.name == '..') return 'file-submodule';
      if (this.type == 'd') return 'file-directory';
      if (this.type == 'f') return 'file';
    },
...
var SVGIcon = {
  view: function(vnode) {
    return m('svg', {
      version: "1.1",
      class: "octicon octicon-" + vnode.attrs.icon + ' ' + vnode.attrs.kind || '',
      "aria-hidden": "true",
    }, m('use', { "xlink:href": "#" + vnode.attrs.icon }))
  },
};
...
var ListHeader = {
  ...
  view: function(vnode) {
    ...
    if (vnode.state.key == state.sortKey) {
      active = 'active';
      var ico = (vnode.state.dir == 'asc') ? 'chevron-up' : 'chevron-down';

      icon = m(SVGIcon, { icon: ico, kind: vnode.state.dir });
    }
  ...
  view: function(vnode) {
    ...
    return [
      m('td',
        ...
        m(SVGIcon, { icon: entry.icon(), kind: entry.kind() })
      ),
      m('td', 
        m('a', {
          href: href,
